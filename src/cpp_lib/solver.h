...
void set_momentum(MomentumPolicy* mp, SolverState& ss){
    ...
    if(MomFixed* pol = dynamic_cast<MomFixed*>(mp)){
        ...
        ss.momentum = min((float)(pow((float)(pol->base_mom * pol->gamma), (float)(floor((float)(ss.iter) / pol->step_size)))), pol->max_mom);
    } else if(MomLinear* pol = dynamic_cast<MomLinear*>(mp)){
        ss.momentum = min(pol->base_mom + (float)(floor((float)(ss.iter) / pol->step_size)) * pol->gamma, pol->max_mom);
    }
}
...
struct TaskSet{
    TaskSet(){}
    vector<Task> forward_tasks;
    vector<Task> backward_tasks;
};

void exe_forward(TaskSet ts){
    for(int i = 0; i < ts.forward_tasks.size(); ++i){
        if(!ts.forward_tasks[i].is_update){
            ts.forward_tasks[i].func();
        } else {
            //update(solver, ts.forward_tasks[i].param);
        }
    }
}

void exe_backward(TaskSet ts){
