...
float* fc2_gd_inputs_0 = fc1_gd_value;
    ...
    for (j = 0 ; j < 100 ; j ++) { 
        for (i = 0 ; i < 250 ; i ++) { 
            fc1_value[j] += fc1_weights[j*100 + i] * fc1_inputs[j*100 + i];
        }
    ...
    for (j = 0 ; j < 10 ; j ++) { 
        for (i = 0 ; i < 100 ; i ++) { 
            fc2_value[j] += fc2_weights[j*10 + i] * fc2_inputs[j*10 + i];
        }
    ...
    labelvalue = label_value[0];
    loss_value[0] -= log(max(loss_prob[labelvalue], 1e-05));

    labelvalue = label_value[0];
    loss_prob[labelvalue] -= 1;

    for (j = 0 ; j < 10 ; j ++) { 
        for (i = 0 ; i < 100 ; i ++) { 
            fc2_gd_inputs[j*10 + i] += fc2_weights[j*10 + i] * fc2_gd_value[j];
        }
        for (i = 0 ; i < 100 ; i ++) { 
            fc2_gd_weights[j*10 + i] += fc2_inputs[j*10 + i] * fc2_gd_value[j];
        }
    ...
    for (j = 0 ; j < 100 ; j ++) { 
        for (i = 0 ; i < 250 ; i ++) { 
            fc1_gd_inputs[j*100 + i] += fc1_weights[j*100 + i] * fc1_gd_value[j];
        }
        for (i = 0 ; i < 250 ; i ++) { 
            fc1_gd_weights[j*100 + i] += fc1_inputs[j*100 + i] * fc1_gd_value[j];
        }
